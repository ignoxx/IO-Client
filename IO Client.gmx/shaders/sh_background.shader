...
uniform vec2 iMouse;                // shader playback time (in seconds)
...
#ifdef ANIMATE
    vec3 nebulaColor1 = hsv2rgb(vec3(.5+.5*sin(iGlobalTime*.1), 0.5, .25));
    vec3 nebulaColor2 = hsv2rgb(vec3(.5+.5*sin(iGlobalTime*.21), 1., .25));
#else
    vec3 nebulaColor1 = hsv2rgb(vec3(.5, 0.5, .25));
    vec3 nebulaColor2 = hsv2rgb(vec3(.7, 1., .25));
#endif
    result += vec3(0.1, 0.0, 0.0); //added myself
